...
The optional interpreter maps latent states back to explicit reasoning. It initializes a separate Qwen3-8B decoder and conditions on weighted top-10 latent targets from the joint encoder.
...
CUDA_VISIBLE_DEVICES=0 python -m latentgrm.interpreter.evaluate \
  ...
  --adapter outputs/interpreter/model/explainer_lora \
  --output outputs/interpreter/evaluation.json \
...
python -m latentgrm.interpreter.score \
  --evaluation outputs/interpreter/evaluation.json \
  --gold-answers data/train.jsonl --output outputs/interpreter/fidelity.json
```
