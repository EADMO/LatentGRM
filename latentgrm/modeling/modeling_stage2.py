...
from latentgrm.modeling.utils import get_input_embeddings
...
def weighted_embedding_from_topk(
    ...
    E = embedding.weight
    topk_indices = topk_indices.to(E.device)
    topk_probs = topk_probs.to(dtype=E.dtype, device=E.device)

    topk_emb = F.embedding(topk_indices, E)
...
class LatentSFTStage2SoftEmbedding(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        embedding = get_input_embeddings(self.latent_model)
        input_embeddings = embedding(ids_for_embed).detach()
        W = embedding
    ...
    def one_example_generate_lora(
        ...
        """Generate a latent chain and then a short visible answer."""
        ...
        next_input_embeddings = None
        reached_latent_end = False

        with torch.no_grad():
            for _ in range(max_new_tokens):
                if next_input_ids is not None:
                    next_input_embeddings = embedding(next_input_ids)
                outputs = self.latent_model(
                    inputs_embeds=next_input_embeddings,
                    attention_mask=attention_mask,
                ...
                if next_token[0,0].item() == self.latent_token_ids[1][0]:
                    reached_latent_end = True
                    break

                latent_token_ids.append(int(next_token.item()))
                topk_logits, topk_indices = torch.topk(
                    next_token_logits,
                    k=min(self.topk_interpolation, next_token_logits.size(-1)),
                    dim=-1,
                )
                if add_gumbel_noise:
                    log_probs = torch.log_softmax(topk_logits.float(), dim=-1)
                    gumbels = -torch.empty_like(log_probs).exponential_(
                        generator=gumbel_generator
                    ).log()
                    noisy_logits = log_probs + noise_scale * gumbels
                    topk_probs = torch.softmax(
                        noisy_logits / gumbel_temperature, dim=-1
                    ).to(embedding_weight.dtype)
                else:
                    topk_probs = torch.softmax(topk_logits.float(), dim=-1).to(
                        embedding_weight.dtype
                    )
                topk_embeddings = F.embedding(topk_indices, embedding_weight)
                next_input_embeddings = (
                    topk_embeddings * topk_probs.unsqueeze(-1)
            ...
            if not reached_latent_end and latent_token_ids:
                # The final predicted latent embedding has not been consumed yet.
                outputs = self.latent_model(
                    inputs_embeds=next_input_embeddings,
                    attention_mask=attention_mask,
                    past_key_values=past_key_values,
            ...
            end_ids = torch.tensor(
                self.latent_token_ids[1],
                dtype=torch.long,
                device=input_ids.device,
            ).unsqueeze(0)
            next_input_embeddings = embedding(end_ids)
            ...
            for _ in range(max_answer_tokens):
                ...
                next_input_embeddings = embedding(next_token)
        ...
        return {
            "text": decoded_text,
            "stopped_early": reached_latent_end,
            "generate_token_num": len(latent_token_ids),
            "answer_token_num": len(answer_token_ids),
        }
    ...
    def one_example_generate_hf(
        ...
        embedding = get_input_embeddings(model)
        E = embedding.weight.detach()
        ...
        with torch.no_grad():
            ...
            for latent_step in range(max_new_tokens):
                ...
                if add_gumbel_noise:
                    ...
                    topk_probs = torch.softmax(noisy_logits / gumbel_temperature, dim=-1).to(E.dtype)
                else:
                    topk_probs = torch.softmax(topk_logits.float(), dim=-1).to(E.dtype)#1:k

                # outputs.logits has already been projected by the LM head W;
                # reconstruct the next latent state from the input basis E.
                topk_emb = F.embedding(topk_indices, E)#1:k
                input_embeddings = (topk_emb * topk_probs.unsqueeze(-1)).sum(dim=1)

                # probs = torch.softmax(next_token_logits, dim=-1)
                # input_embeddings = probs @ E
            ...
            if latent_states:
                ...
                latent_state = E.new_empty(
                    (inputs['input_ids'].size(0), 0, E.size(1))
                )

            think_ids = torch.LongTensor(model.latent_token_ids[1]).unsqueeze(0).to(model.device)
            think_end_embeddings = embedding(think_ids)
            input_ids = inputs['input_ids']
