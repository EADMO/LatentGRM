...
class ModelConfig:
    ...
    """Latent-SFT (Chain of Superposition) mode. During generation, the hidden
    state at each latent position is projected to a top-k soft embedding
    (z = E[topk_idx] @ topk_probs, where E is the input embedding matrix) and fed
    back as the input embedding for the next step instead of a hard token id.
    Implies enable_prompt_embeds=True."""
    latent_end_token_id: int | None = None
    """Token ID that ends the latent chain (e.g. the `</think>` id). When the
    model emits this token, decoding switches from latent mode to normal text
    generation."""
    latent_topk: int = 10
    """Number of top vocabulary tokens kept when projecting latent hidden states
    to a soft embedding (top-k softmax over the LM head logits)."""
