...
import functools
from vllm.v1.worker.gpu import latent_fast
import gc
import time
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Latent-SFT (Chain of Superposition) mode.
        self.enable_latent = self.model_config.enable_latent
        if self.enable_latent and (
            ...
            raise ValueError("Latent fast path requires PP=PCP=DCP=1 and no speculation")
        self.latent_end_token_id = self.model_config.latent_end_token_id
        self.latent_topk = self.model_config.latent_topk
        self._latent_embed_weight_full: torch.Tensor | None = None
    ...
    def add_requests(self, scheduler_output: SchedulerOutput) -> None:
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            if self.enable_latent:
                self.req_states.latent_mode[req_index] = True
                if sampling_params is not None:
                    self.req_states.latent_sampling_params[req_index] = (
                        sampling_params
                    )
        ...
        if self.enable_latent and scheduler_output.scheduled_new_reqs:
            latent_fast.initialize_requests(self, [
                self.req_states.req_id_to_index[req.req_id]
    ...
    def _latent_embed_weight(self) -> torch.Tensor:
        """Full-vocab embedding matrix used to build soft embeddings.

        Under tensor parallelism the embedding is sharded across TP ranks;
        weights do not change during decode, so we all-gather once and cache."""
        cached = self._latent_embed_weight_full
        if cached is not None:
            return cached
        model = self.model
        weight = None
        if hasattr(model, "embed_tokens") and hasattr(
            getattr(model, "embed_tokens"), "weight"
        ):
            weight = getattr(model, "embed_tokens").weight
        elif hasattr(model, "model"):
            sub = getattr(model, "model")
            if hasattr(sub, "embed_tokens"):
                weight = getattr(sub, "embed_tokens").weight
        if weight is None:
            raise AttributeError(
                "Latent mode requires an input embedding matrix (embed_tokens)"
            )
        ...
        self._latent_embed_weight_full = weight
        return weight

    def _latent_project(self, input_batch, logits):
        return latent_fast.project(self, input_batch, logits)
    ...
    def sample(
        ...
        sample_hidden_states = hidden_states[input_batch.logits_indices]
        if self.enable_latent:
            latent_result = latent_fast.sample_hidden(self, sample_hidden_states, input_batch)
            if latent_result is not None:
                return latent_result
        logits = self.model.compute_logits(sample_hidden_states)
        latent_override = None
        if self.enable_latent:
            latent_override = self._latent_project(input_batch, logits)
        ...
        if latent_override is not None:
            latent_fast.apply_sample(self, input_batch, sampler_output, latent_override)
    ...
    def execute_model(
        ...
        if self.supports_mm_inputs and self.is_first_pp_rank:
            ...
            if inputs_embeds is not None and not self.model.requires_raw_input_tokens:
                input_ids = None
        if self.enable_latent and self.is_first_pp_rank:
            inputs_embeds = latent_fast.prepare_embeds(self, input_batch)
            input_ids = None
