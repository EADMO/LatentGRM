...
class RequestState:
    def __init__(
        ...
        # Latent-SFT (Chain of Superposition) per-request state.
        # latent_mode: whether the request is still generating the latent chain.
        self.latent_mode = np.zeros(self.max_num_reqs, dtype=bool)
        # req_state_idx -> soft embedding z=E[topk]@probs for the next latent step.
        self.latent_prev_soft_embedding: dict[int, torch.Tensor] = {}
        # req_state_idx -> SamplingParams (for Gumbel options).
        self.latent_sampling_params: dict[int, object] = {}
        # req_state_idx -> torch.Generator seeded from SamplingParams.seed, so
        # Gumbel noise is reproducible across runs (matches HF backend).
        self.latent_generators: dict[int, torch.Generator] = {}
    ...
    def add_request(
        ...
        self.latent_mode[req_idx] = False
        self.latent_prev_soft_embedding.pop(req_idx, None)
        self.latent_sampling_params.pop(req_idx, None)
        self.latent_generators.pop(req_idx, None)
