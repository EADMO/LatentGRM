...
def check_stop(request: Request, max_model_len: int) -> bool:
    ...
    if request.latent_mode:
        # Latent-SFT: never stop on eos/stop tokens while generating the
        # latent chain (its projected tokens are not user-visible text).
        # When the latent-end token is emitted, switch to text mode and
        # keep decoding. max_tokens / max_model_len remain safety nets.
        latent_end = getattr(request, "latent_end_token_id", None)
        if latent_end is not None and request.output_token_ids[-1] == latent_end:
            request.latent_mode = False
        if (
