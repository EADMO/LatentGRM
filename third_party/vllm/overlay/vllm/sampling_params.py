...
class SamplingParams(
    ...
    """Per-request override of the latent-chain end token id (e.g. the
    `</think>` token id). Only relevant when latent mode is enabled."""
    gumbel_softmax_temperature: float = 1.0
    """Temperature of the Gumbel-softmax used to build latent soft embeddings."""
    noise_scale: float = 1.0
    """Scale of the Gumbel noise added to latent logits."""
    add_noise_gumbel_softmax: bool = False
    """Whether to add Gumbel noise when projecting latent logits to a soft
    embedding (used to diversify latent rollouts)."""
    use_one_sided_gumbel_noise: bool = False
    """If True, only keep positive Gumbel noise (one-sided variant)."""
    ...
    def from_optional(
        ...
        logprob_token_ids: list[int] | None = None,
        latent_end_token_id: int | None = None,
        gumbel_softmax_temperature: float | None = 1.0,
    ...
    def __post_init__(self) -> None:
        if self.add_noise_gumbel_softmax and self.gumbel_softmax_temperature <= 0:
            raise ValueError(
                "gumbel_softmax_temperature must be positive when "
                "add_noise_gumbel_softmax is enabled"
            )
    ...
    def _verify_greedy_sampling(self) -> None:
        latent_noise = (self.add_noise_gumbel_softmax
                        and self.latent_end_token_id is not None)
        if self.n > 1 and not latent_noise:
